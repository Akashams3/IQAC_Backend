...
import com.iqac.project.dto.MaterialResponse;
...
public class MaterialService {
    ...
    public void create(MultipartFile file, Long facultyId, Long deptId, MaterialRequest req) throws IOException {
        ...
        Material material = Material.builder()
                ...
                .academicYear(AcademicYearUtil.normalizeToShort(AcademicYearUtil.blankToNull(req.getAcademicYear())))
    ...
    public List<MaterialResponse> getMy(Long facultyId) {
        log.info("Fetching materials for faculty={}", facultyId);
        List<Material> materials = repo.findByFacultyId(facultyId);
        return materials.stream()
                .map(m -> MaterialResponse.builder()
                        ...
                        .facultyName(m.getFaculty() != null ? m.getFaculty().getFacultyName() : "Unknown")
                        .build())
                .collect(Collectors.toList());
    }

    public List<MaterialResponse> getAll(Long deptId, String academicYear, String semester, String status) {
        ...
        List<Material> materials = repo.findByFilters(deptId, yp[0], yp[1], semester, statusEnum);
        return materials.stream()
                .map(m -> MaterialResponse.builder()
                        .id(m.getId())
                        .title(m.getTitle())
