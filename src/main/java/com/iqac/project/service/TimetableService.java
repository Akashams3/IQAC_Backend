...
public class TimetableService {
    ...
    public List<Timetable> getAll(Long departmentId, String academicYear, String semester) {
        ...
        if (yp[0] != null || AcademicYearUtil.blankToNull(semester) != null) {
            return timetableRepository.findByDepartmentFlexible(departmentId, yp[0], yp[1], semester);
        }
    ...
    public void uploadExcel(MultipartFile file, Long departmentId,
        ...
        academicYear = AcademicYearUtil.normalizeToShort(AcademicYearUtil.blankToNull(academicYear));
        if (academicYear == null)
            throw new RuntimeException("Academic year is required");

        String[] yp = AcademicYearUtil.filterPair(academicYear);
        if (timetableRepository.countByDepartmentSemesterAndAcademicYearEither(departmentId, yp[0], yp[1], semester) > 0)
    ...
    public void updateBySlot(Long deptId, String year, String sem,
        ...
        String ny = AcademicYearUtil.normalizeToShort(AcademicYearUtil.blankToNull(year));
        String[] yp = AcademicYearUtil.filterPair(ny);
        Timetable tt = timetableRepository
                .findSlotByDeptSemDayPeriodYearEither(deptId, sem, day, period, yp[0], yp[1])
    ...
    public byte[] download(Long deptId, String academicYear, String semester) throws IOException {
        ...
        String[] yp = AcademicYearUtil.filterPair(academicYear);
        List<Timetable> list = timetableRepository.findByDepartmentFlexible(deptId, yp[0], yp[1], semester);
        if (list.isEmpty())
    ...
    public void deleteByFilter(Long deptId, String academicYear, String semester) {
        ...
        String[] yp = AcademicYearUtil.filterPair(academicYear);
        if (timetableRepository.countByDepartmentSemesterAndAcademicYearEither(deptId, yp[0], yp[1], semester) == 0)
            throw new ResourceNotFoundException("No data to delete");
        timetableRepository.deleteByDepartmentIdAndAcademicYearAndSemester(deptId, yp[0], yp[1], semester);
